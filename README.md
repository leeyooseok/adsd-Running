...
[![Video Title](https://img.youtube.com/vi/x32PiAAVnLo)](https://www.youtube.com/shorts/x32PiAAVnLo))
...
![image](https://github.com/user-attachments/assets/89d08f88-5b86-4c9c-a992-3bd2636189c7)


- _04_cnn_training_5는 이미지 분류 모델을 사용하여 테스트 데이터에 대한 예측 결과를 시각화하는 내용입니다
- 이전에 학습된 모델을 파일로부터 로드합니다. 테스트 데이터를 모델에 넣어 예측하였고 결과는 소프트맥스 확률 형태로 반환되며 각 클래스에 대해 모델이 예측하는 확률 값을 가집니다.
- 16개의 테스트 데이터 샘플을 무작위로 선택하여 4x4형식의 이미지로 나타냅니다.
- 이 코드의 시각화는 테스트 이미지에 대한 모델 예측 결과를 바로 보여주어 예측의 정확도를 시각적으로 구분하도록 하였습니다.

## _05_ai_driving.py

- _05_ai_driving은 실시간 비디오 스트리밍을 처리하고, CNN 모델을 이용해 명령을 분류한 후, 시리얼 통신을 통해 모터에 명령을 전달하는 시스템을 구성합니다.
- OpenCV를 사용해 실시간으로 비디오 스트리밍 데이터를 처리하고, CNN 모델에 입력할 수 있도록 전처리하였습니다.
- 큐(queue)를 활용해 비디오 프레임을 효율적으로 관리하고 멀티스레딩(threading)을 통해 실시간 비디오 처리와 CNN 예측이 동시에 이루어지도록 구성하였습니다.
